...
class LevelComplete extends StatekitView<LevelCompleteController> implements LevelCompleteBinding {
      ...
      body: GameTableBackground(
        child: Stack(
          children: [
            ...
            const Center(child: SunburstRays(size: 600)),

            // Ambient Celebration Confetti and Star Sparkles
            const Positioned.fill(child: ConfettiParticles()),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                VictoryRibbon(
                                  title: data.stars >= 3 ? 'VICTORY!' : 'LEVEL COMPLETE!',
                                  levelNumber: data.levelNumber,
                                ),
                                const SizedBox(height: 12),

                                // Animated 3-Star Celebration Arc
                                AnimatedStarRating(stars: data.stars, startDelay: const Duration(milliseconds: 350)),
                                const SizedBox(height: 12),

                                // Main Stats & Score Parchment Plaque
                                ConstrainedBox(
                                  constraints: BoxConstraints(maxWidth: (maxWidth * 0.9).clamp(280.0, 420.0)),
                                  child: LevelStatsCard(data: data, startDelay: const Duration(milliseconds: 700)),
                                ),
                                SizedBox(height: isCompact ? 20 : 30),

                                // Big Juicy "NEXT LEVEL ▶" Primary Button
                                ConstrainedBox(
                                  constraints: BoxConstraints(maxWidth: (maxWidth * 0.85).clamp(260.0, 380.0)),
                                      ...
                                      children: [
                                        Text(
                                          ...
                                          style: TextStyle(
                                            ...
                                            letterSpacing: 1.6,
                                            shadows: [
                                              Shadow(color: Color(0xFF003300), blurRadius: 4, offset: Offset(0, 2)),
                                            ],
                                          ),
                                        ),
                                        SizedBox(width: 8),
    ...
    Navigator.pushReplacement(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (context, animation, secondaryAnimation) => StateProvider(
          stateProvider: StatekitProvider(create: () => PlayScreenController(level: nextLevel)),
          child: PlayScreen(),
        ),
  ...
  void onReplayLevel(int levelNumber) {
    Navigator.pushReplacement(
      ...
      PageRouteBuilder(
        ...
        pageBuilder: (context, animation, secondaryAnimation) => StateProvider(
          stateProvider: StatekitProvider(create: () => PlayScreenController(level: levelNumber)),
          child: PlayScreen(),
        ),
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          final curved = CurvedAnimation(parent: animation, curve: Curves.easeOutCubic);
  ...
  void onLevelsPressed() {
    Navigator.pushNamedAndRemoveUntil(context, Routes.levelSelection, (route) => route.isFirst);
  }
